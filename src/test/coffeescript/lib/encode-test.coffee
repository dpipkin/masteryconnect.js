...
describe('encode', ->
	describe('authorizationHeader()', ->
		it('should return string representation of an authorization', ->
			oauth.authorization('GET', 'url', {}, 'consumerKey', 'consumerSecret', 'token', 'tokenSecret').cata(
				success: (auth) ->
					encoded = encode.authorizationHeader(auth).getOrElse('')
	...
	describe('rfc3986()', ->
		it('should return RFC 3986 compatible string', ->
			encode.rfc3986('hello there!').getOrElse('').should.equal('hello%20there%21')
		)
	)
	describe('url()', ->
		it('should return string representation of a map', ->
			encode.url(test2: 'test2', test1: 'test1').getOrElse('').should.equal('test1=test1&test2=test2')
		)
	)
)
